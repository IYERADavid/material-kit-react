...
import { CustomersTable } from '@/components/dashboard/customer/digitalCards-table';
import type { Customer } from '@/components/dashboard/customer/digitalCards-table';
...
export default function Page(): React.JSX.Element {
  ...
  const [digitalCards, setDigitalCards] = React.useState([]);
  ...
  React.useEffect(() => {
    async function getLoggedInUserData() {
      if(user?.id){
        const digitalCards = await digitalCardClient.getDIgitalCards(user.id);
        console.log(digitalCards);
        setDigitalCards(digitalCards)
      }
    }
    ...
    <Stack spacing={3}>
      <Stack direction="row" spacing={3}>
        <Stack spacing={1} sx={{ flex: '1 1 auto' }}>
          <Typography variant="h4">Digital Cards</Typography>
        ...
        <div>
          <Button component={RouterLink} href={paths.dashboard.digitalCards + '/add'} startIcon={<PlusIcon fontSize="var(--icon-fontSize-md)" />} variant="contained">
            Add
          </Button>
        </div>
      </Stack>
